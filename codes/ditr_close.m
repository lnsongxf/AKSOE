...
%%Analyze stability in the close economy for the Domestic (current 
...
LOAD_OLD = 1; % load saved results ('diter.mat' and 'ditre.mat')
...
if LOAD_OLD == 1
    ...
    save('ditr_close.mat', 'stable1', 'deter', 'COMB');
end
...
save('ditre_close.mat', 'stable2', 'detere');
end
...
% title('Close economy with FB-DITR');
% xlabel('policy reaction to inflation');
% ylabel('policy reaction to output-gap');
% 
% hold off
...
figure
    title('Closed economy with DITR');
    xlabel('\phi_{\pi}');
    ylabel('\phi_{x}');
    axis([0 xmax 0 ymax])
...
figure
    title('Closed economy with FB-DITR');
    xlabel('\phi_{\pi}');
    ylabel('\phi_{x}');
    axis([0 xmax 0 ymax])
